...
<body>
                        ...
                        <tbody>
                            ...
                            <tr style="border-bottom: 1px solid #f3f4f6;">
                                <td style="padding: 16px;">
                                    <img src="{{ $berita->thumbnail_url }}" alt="{{ $berita->judul }}" class="thumbnail">
                                </td>
                                <td style="padding: 16px;">
                                    <span style="font-size: 14px; font-weight: 600; color: #111827;">{{ Str::limit($berita->judul, 50) }}</span>
                                </td>
                                <td style="padding: 16px;">
                                    @if($berita->kategori)
                                    <span style="display: inline-block; padding: 4px 12px; background: #FACC15; color: #000; border-radius: 12px; font-size: 12px; font-weight: 500;">{{ $berita->kategori }}</span>
                                    @else
                                    <span style="font-size: 12px; color: #9ca3af;">-</span>
                                    @endif
                                </td>
                                <td style="padding: 16px;">
                                    <p style="font-size: 14px; color: #6b7280; margin: 0; line-height: 1.5;">{{ Str::limit($berita->excerpt, 100) }}</p>
                                </td>
                                <td style="padding: 16px; font-size: 14px; color: #6b7280;">{{ formatTanggalSingkat($berita->tanggal) }}</td>
                                <td style="padding: 16px;">
                                    <div style="display: flex; justify-content: center; gap: 8px;">
                                        <a href="{{ route('admin.berita.show', $berita) }}" style="padding: 8px; background: transparent; border: none; color: #10b981; cursor: pointer; border-radius: 6px; display: inline-block;" title="Lihat Detail">
                                            <svg style="width: 16px; height: 16px;" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                                            </svg>
                                        </a>
                                        <a href="{{ route('admin.berita.edit', $berita) }}" style="padding: 8px; background: transparent; border: none; color: #2563eb; cursor: pointer; border-radius: 6px; display: inline-block;" title="Edit">
                                            <svg style="width: 16px; height: 16px;" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" />
                                            </svg>
                                        </a>
                                        <form action="{{ route('admin.berita.destroy', $berita) }}" method="POST" style="display: inline-block;" onsubmit="showDeleteConfirmation(this, event); return false;">
                                            @csrf
                                            @method('DELETE')
    ...
    {{-- Include Loading & Confirmation Modals --}}
